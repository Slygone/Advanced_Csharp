...
//relative path to consoleapp folder
...
Console.WriteLine($"My folder exists: {myFolderExist}");
...
{
    //overwrite the content of the test.txt
...
if (File.Exists(appPath + @"myFolder\test.txt"))
{
    //append the content of the test.txt
    File.AppendAllText(appPath + @"myFolder\test.txt", Environment.NewLine + "Goodbye SEDC!");
}

//delete
if(File.Exists(appPath + @"myFolder/test.txt"))
{
    File.Delete(appPath + @"myFolder/test.txt");
}